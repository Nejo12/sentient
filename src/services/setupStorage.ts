...
export const SETUP_OVERLAY_DONE_KEY = 'setup_overlay_done';
...
export async function isOverlaySetupDone(): Promise<boolean> {
  const value = await AsyncStorage.getItem(SETUP_OVERLAY_DONE_KEY);
  return value === 'true';
}

export async function setOverlaySetupDone(): Promise<void> {
  await AsyncStorage.setItem(SETUP_OVERLAY_DONE_KEY, 'true');
}
